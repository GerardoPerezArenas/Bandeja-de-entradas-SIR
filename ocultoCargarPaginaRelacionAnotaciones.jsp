...
//variable estado2 que es la que aparece en la columna de la tabla estado y necesita el estado pendiente
   ...
   if("<%= relaciones%>"=='relaciones'){
       ...
       parent.mainFrame.lista[<%= z++ %>]  = [estado+ejer+"/"+numAnot,"<%= cp_anotacion.elementAt(5) %>",
           ...
           var esEntradaSIR = ("<%= cp_anotacion.elementAt(20) %>" === "1");
       var estadoSIR = "";
          var numRegistro = estado+ejer+"/"+numAnot;

       if (esEntradaSIR) {
              var codigoEstadoSIR = "<%= cp_anotacion.elementAt(25) != null ? cp_anotacion.elementAt(25) : "" %>";

              // Mapa código → descripción (usando descriptor)
              var ESTADOS_SIR = {
                  "": '',
              ...
              // Usar la descripción si existe, si no mostrar el código
              estadoSIR = ESTADOS_SIR[codigoEstadoSIR] || codigoEstadoSIR;

              numRegistro = numRegistro + "<br>" + "<%= cp_anotacion.elementAt(24) != null ? cp_anotacion.elementAt(24) : "" %>";
          }
       ...
       parent.mainFrame.lista[<%= j++ %>]  = [check,(esEntradaSIR ? estadoSIR : estado2),procedimiento,expediente,
                      "<%= cp_anotacion.elementAt(19) %>",numRegistro,entrSal,"<%= cp_anotacion.elementAt(5) %>",
