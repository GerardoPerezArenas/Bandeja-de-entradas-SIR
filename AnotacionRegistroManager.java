 ...
 public String consultaXML(GeneralValueObject gVO,String[] params)  throws AnotacionRegistroException {
    ...
    public RegistroValueObject getCampoAnotacionByPrimaryKey(RegistroValueObject registro, String campo, String[] params)
     ...
     public Vector<String> getDatosAnotacion(RegistroValueObject registro, String[] params)
        ...
        try {
            ...
            resultado.add(registroVO.getEstadoSIR() != null ? String.valueOf(registroVO.getEstadoSIR()) : null); // estadoSIR nuevo
            resultado.add(null);//relleno
            ...
            m_Log.debug("Asiento recuperado");
            m_Log.debug("getDatosAnotacion completo ? " + resultado);
        } catch (TechnicalException te) {
            m_Log.error("JDBC Technical problem " + te.getMessage());
            throw new AnotacionRegistroException("Problema técnico de JDBC " + te.getMessage());
        } catch (BDException bde) {
        ...
        try {
            ...
            resultado.add(registroVO.getNomUniRegDestino());//destino
            resultado.add(String.valueOf(registroVO.getEstAnotacion()));//estado
            resultado.add(registroVO.getEstadoSIR() != null ? String.valueOf(registroVO.getEstadoSIR()) : null);
            resultado.add(null);//relleno
            resultado.add(null);//15-relleno
            resultado.add(null);//relleno
            resultado.add(null);//relleno
   ...
   ****************************************************************************************************** */
    ...
    public int getNumeroTotalAnotaciones(RegistroValueObject patron, String[] params) throws AnotacionRegistroException {
        // Comprobación adicional por consola (por si falla m_Log)
        System.out.println(">>> [MANAGER] getNumeroTotalAnotaciones() - codigoUnidadDestinoSIR = " + patron.getCodigoUnidadDestinoSIR());

        // Log de entrada
        if (m_Log != null) {
            m_Log.info(">>> [INFO] En Manager: getNumeroTotalAnotaciones()");
            m_Log.info(">>> [INFO] codigoUnidadDestinoSIR = " + patron.getCodigoUnidadDestinoSIR());
            m_Log.info(">>> [INFO] identificadorRegistroSIR = " + patron.getIdentificadorRegistroSIR());
            m_Log.info(">>> [INFO] estadoSIR = " + patron.getEstadoSIR());
        } else {
            System.out.println(">>> [ADVERTENCIA] m_Log es null en Manager");
        }
        ...
        try {
            ...
            if (m_Log != null) {
                m_Log.error("JDBC Technical problem: " + ce.getMessage(), ce);
            } else {
                System.err.println(">>> [ERROR] Problema técnico de JDBC: " + ce.getMessage());
            }
            throw new AnotacionRegistroException("Problema técnico de JDBC: " + ce.getMessage());
        }
    }
